...
seu <- subset(seu, subset = nCount_RNA > 1000 & nFeature_RNA < 2500 & percent.mt < 12)     # nCount>1000, mt<12 in the paper (the upper limit of nFeature was not clear)
...
seu <- SCTransform(seu, vars.to.regress = c("percent.mt"), 
                   variable.features.n = 2000    # 2000 in the paper
)

seu <- RunPCA(seu, npcs = 15, verbose = FALSE)    # npcs = 15 in the paper
...
seu <- FindNeighbors(seu, reduction = "harmony", dims = 1:15, verbose = FALSE)
seu <- FindClusters(seu, reduction = "harmony", resolution = 1, verbose = FALSE)
seu <- RunUMAP(seu, reduction = "harmony", dims = 1:15, verbose = FALSE)
...
file.copy(from=plots.png.paths, to="plot/SCT_w_harmony/ver1.1")

# saveRDS(seu, file = "RDSfiles/seu_03_SCT-harmony_1.RDS")
